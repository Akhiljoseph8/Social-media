...
//function for handling post creation
  ...
  const addComment = async (postId) => {
    ...
    const result = await addCommentApi(commentData, header); 
    if (result.status === 200) {
      ...
      setPosts((prevPosts) =>
        prevPosts.map((post) =>
          post._id === postId
            ? { ...post, comments: [...post.comments, result.data.comments] }
            : post
        )
      );
          ...
          <Box sx={{ flexGrow: 1, marginLeft: isLargeScreen ? "350px" : 0 }}>
            <Box
              sx={{
                position: "fixed",
                top: 60, 
                width: isLargeScreen ? "calc(100% - 350px)" : "100%", 
                zIndex: 100,
            ...
            {posts.map((post) => (
              <Card
                key={post.id}
                sx={{ marginBottom: 4, marginTop: 5, padding: 2 }}
              >
                <CardHeader
                ...
                <CardActions>
                  <IconButton
                    ...
                    sx={{
                      color: post.likedBy.includes(userId) ? "red" : "defult",
                    }}
                  >
